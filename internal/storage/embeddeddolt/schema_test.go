...
func TestSchemaAfterInit(t *testing.T) {
	...
	// key (the #4259 failure class). Scanning COLUMN_DEFAULT (rather than
	// counting) also fails if the table or column is missing entirely.
	for _, table := range []string{
		...
		if columnDefault.Valid {
			t.Errorf("%s.id has DEFAULT %q, want none (migration 0051 / ignored 0010)", table, columnDefault.String)
		}
	}
}
